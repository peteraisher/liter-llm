...
// alef:hash:4769f518a9a44a13b50a8876feab2d91023ff136067841413bdeaafaf6159d21
...
fn test_anthropic_stream() {
    ...
    assert_eq!(chunks.iter().map(|c| c.choices.first().and_then(|ch| ch.delta.content.as_deref()).unwrap_or("")).collect::<String>(), "One Two Three");
    assert!(chunks.last().and_then(|c| c.choices.first()).and_then(|ch| ch.finish_reason.as_ref()).is_some(), "expected true");
    });
}

#[test]
fn test_azure_stream() {
    common::runtime().block_on(async {
    ...
    let chunks: Vec<_> = tokio_stream::StreamExt::collect::<Vec<_>>(stream).await
        ...
        .collect();
    assert!(chunks.len() >= 3 as usize, "expected >= 3 chunks");
    assert_eq!(chunks.iter().map(|c| c.choices.first().and_then(|ch| ch.delta.content.as_deref()).unwrap_or("")).collect::<String>(), "1 2 3");
    assert!(chunks.last().and_then(|c| c.choices.first()).and_then(|ch| ch.finish_reason.as_ref()).is_some(), "expected true");
    });
}
...
#[test]
fn test_edge_stream_function_call() {
    common::runtime().block_on(async {
...
fn test_empty_stream() {
    common::runtime().block_on(async {
        ...
        let req_json: serde_json::Value = serde_json::from_str(
            r#"{"messages":[{"content":"Say nothing","role":"user"}],"model":"gpt-4","stream":true}"#,
        )
        .unwrap();
        let req = serde_json::from_value(req_json).unwrap();
        let client =
...
#[test]
fn test_stream_content_policy_error() {
    common::runtime().block_on(async {
    ...
    { let __e = result.as_ref().err().unwrap(); assert!(format!("{:?}", __e).contains("ContentPolicy") || __e.to_string().contains("ContentPolicy"), "error message mismatch"); }
    assert_eq!({ let __err = result.as_ref().err().unwrap(); __err.status_code() }, 400, "equals assertion failed");
    });
}

#[test]
fn test_stream_done_signal() {
    common::runtime().block_on(async {
    ...
    let chunks: Vec<_> = tokio_stream::StreamExt::collect::<Vec<_>>(stream).await
        ...
        .collect();
    assert!(chunks.last().and_then(|c| c.choices.first()).and_then(|ch| ch.finish_reason.as_ref()).is_some(), "expected true");
    assert_eq!(chunks.iter().map(|c| c.choices.first().and_then(|ch| ch.delta.content.as_deref()).unwrap_or("")).collect::<String>(), "Done");
    assert!(true, "expected true");
    });
}

#[test]
fn test_stream_error_401() {
    common::runtime().block_on(async {
...
#[test]
fn test_stream_multiple_choices() {
    common::runtime().block_on(async {
...
#[test]
fn test_stream_with_tool_calls() {
    common::runtime().block_on(async {
    ...
    assert_eq!(chunks.last().and_then(|c| c.choices.first()).and_then(|ch| ch.finish_reason.as_ref()).map(|v| v.to_string()).unwrap_or_default(), "tool_calls");
    assert!(!chunks.iter().flat_map(|c| c.choices.iter().flat_map(|ch| ch.delta.tool_calls.iter().flatten())).collect::<Vec<_>>().is_empty(), "expected non-empty");
    assert_eq!(chunks.iter().flat_map(|c| c.choices.iter().flat_map(|ch| ch.delta.tool_calls.iter().flatten())).nth(0).and_then(|x| x.function.as_ref()).and_then(|x| x.name.as_deref()).unwrap_or(""), "get_weather");
    });
}

#[test]
fn test_vertex_stream() {
    common::runtime().block_on(async {
