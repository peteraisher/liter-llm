...
import { Content as Snip_python_guides_configuration } from "../../../snippets/python/configuration/custom-base-url.md";
import { Content as Snip_typescript_guides_configuration } from "../../../snippets/typescript/configuration/custom-base-url.md";
import { Content as Snip_rust_usage_configuration } from "../../../snippets/rust/configuration/custom-base-url.md";
import { Content as Snip_go_guides_configuration } from "../../../snippets/go/configuration/custom-base-url.md";
import { Content as Snip_java_usage_configuration } from "../../../snippets/java/configuration/custom-base-url.md";
import { Content as Snip_csharp_usage_configuration } from "../../../snippets/csharp/configuration/custom-base-url.md";
import { Content as Snip_ruby_usage_configuration } from "../../../snippets/ruby/configuration/custom-base-url.md";
import { Content as Snip_php_usage_configuration } from "../../../snippets/php/configuration/custom-base-url.md";
import { Content as Snip_elixir_usage_configuration } from "../../../snippets/elixir/configuration/custom-base-url.md";
import { Content as Snip_wasm_usage_configuration } from "../../../snippets/wasm/configuration/custom-base-url.md";
...
| Field           | Type | Description                                                   |
| --------------- | ---- | ------------------------------------------------------------- |
| `max_in_flight` | int  | Maximum outstanding provider requests. Must be greater than 0. |
...
:::note[AWS Bedrock]
Bedrock takes no API key here. Its credentials are read on every request instead: a Bedrock API key from `AWS_BEARER_TOKEN_BEDROCK`, or, with the `bedrock` feature, an access-key pair from `AWS_ACCESS_KEY_ID` + `AWS_SECRET_ACCESS_KEY`. See [Authentication](/usage/authentication/).
:::
...
Bound simultaneous provider requests across all operations that share a managed client. Requests wait for a permit; cached responses do not consume one.
...
```rust
use liter_llm::InFlightLimitConfig;

let config = InFlightLimitConfig {
    max_in_flight: Some(8),
};
let builder = builder.in_flight_limit(config);
```
...
`liter-llm.toml` can declare custom providers, but loading the file does **not** register them automatically — `FileConfig::into_builder()` only maps client-level settings onto `ClientConfigBuilder`. Read the entries back with `FileConfig::providers()` and call `register_custom_provider` yourself for each one:
...
```rust
use liter_llm::{register_custom_provider, AuthHeaderFormat, CustomProviderConfig, FileConfig};
