...
| `bedrock`       | no      | AWS SigV4 signing in `BedrockProvider`. Without it the provider still routes `bedrock/` model names and authenticates with `AWS_BEARER_TOKEN_BEDROCK`, but cannot sign.      | `aws-sigv4`, `aws-credential-types`                             |
