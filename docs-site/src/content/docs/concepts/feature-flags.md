...
| `bedrock`       | no      | AWS SigV4 request signing in `BedrockProvider`. Without this flag, the provider still routes `bedrock/` model names and can authenticate with a Bedrock API key (`AWS_BEARER_TOKEN_BEDROCK`), but cannot sign with an access-key pair. | `aws-sigv4`, `aws-credential-types`                             |
